...
class ShahidBaseIE(AWSIE):
    ...
    # Public Shahid API gateway key (redacted for GitHub push protection; restore from upstream yt-dlp if needed)
    _AWS_API_KEY = ''
    ...
    def _call_api(self, path, video_id, request=None):
        ...
        try:
            return self._aws_execute_api({
                'uri': '/proxy/v2/' + path,
                # Public Shahid AWS IE credentials (redacted for GitHub push protection; restore from upstream yt-dlp if needed)
                'access_key': '',
                'secret_key': '',
            }, video_id, query)
