...
- `check`: open one profile and report whether the DM button is visible.
...
- Success is recorded only when the composer clears and the approved message appears in the conversation. Otherwise the attempt is marked uncertain and retries are blocked for manual reconciliation.
