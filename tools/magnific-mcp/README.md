...
4. Completion emits an agent event instructing Arisa to run `collect-generation` with delivery enabled for every undelivered index.
5. Each collection records a fail-closed delivery attempt before returning the artifact. An uncertain delivery is never retried automatically. Later independent checks stop after every image has an attempt recorded.
