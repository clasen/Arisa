...
| [`real-video-analyzer`](./real-video-analyzer/) | Prepares video evidence with scene-aware keyframes, visual deduplication, transcript data, and a viewer | video + `text/plain` URL | `application/zip` | Python 3 + `zip` | High — bundled Python video-analysis environment |
