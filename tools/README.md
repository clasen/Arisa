...
| [`roster-sites`](./roster-sites/) | Manages and publishes small HTTPS sites through RosterServer (start/stop daemon, init, publish, list, DNS) | text + HTML | `application/json` | Public VPS + DNS + ports 80/443 (Let's Encrypt) | Medium — 2 npm deps + RosterServer daemon |
