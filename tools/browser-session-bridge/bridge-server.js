...
function boundedRequestHeader(request, name, maxLength) {
  const raw = request.headers[name];
  const value = (Array.isArray(raw) ? raw.join(", ") : String(raw || "")).replace(/[\u0000-\u001f\u007f]/g, "").trim();
  return value ? value.slice(0, maxLength) : null;
}

function requestBrowserIdentity(request) {
  const fields = {
    userAgent: boundedRequestHeader(request, "user-agent", 512),
    acceptLanguage: boundedRequestHeader(request, "accept-language", 256),
    clientHints: boundedRequestHeader(request, "sec-ch-ua", 256),
    platformHint: boundedRequestHeader(request, "sec-ch-ua-platform", 64),
    mobileHint: boundedRequestHeader(request, "sec-ch-ua-mobile", 16)
  };
