  ...
  try {
    ...
    if (mode.startsWith("session-")) {
      ...
      if (mode === "session-open" || mode === "session-open-authenticated") {
        ...
        const deviceId = String(request.args?.deviceId || "").trim();
        if (mode === "session-open-authenticated" && !resourceId) return toolError("session-open-authenticated requires resourceId.");
        const output = await daemon.submit({ action: mode, ...(resourceId ? { resourceId } : {}), ...(deviceId ? { deviceId } : {}) }, { timeoutMs, readyTimeoutMs });
