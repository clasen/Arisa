...
async function createStartedClient(binary, config) {
  const operationTimeoutMs = boundedInteger(config.TIMEOUT_MS, 30_000, 5_000, 60_000);
  const client = new McpProcess(binary, buildMcpCommand(config, operationTimeoutMs), {
    timeoutMs: 0,
...
export function createPersistentSessionService({ binary, config, createClient = null, now = Date.now, lookup = null, tmpDir = null }) {
  const manager = new BrowserSessionManager({
    createClient: createClient || (() => createStartedClient(binary, config)),
    ...
    if (action === "session-call") {
      ...
      try {
        ...
        return {
          ...
          permission,
          text: bounded.text,
          finalUrl: await finalPublicUrl(payload.sessionId, signal),
