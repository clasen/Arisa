...
| `creator-scout` auth/status/search | Migrated in `creator-scout` 1.3.0. A separate Panda profile completed email magic-link authentication, same-site callback consumption, cookie/storage persistence, `/saved` revalidation, exact-title search, and bounded email reveal. | Keep checkout, Stripe, and subscription inspection explicitly on Chromium. No automatic fallback. |
