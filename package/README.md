...
Tools may declare weighted execution resources in their manifest, for example `"execution": { "resourceClass": "browser", "weight": 1 }`. Runs sharing a declared class queue fairly once they reach its capacity; undeclared lightweight tools remain unconstrained. Browser and orchestrator classes default to capacity one, while other declared classes default to two. Before starting declared work, Arisa checks host memory, swap use, and worker RSS. Unsafe pressure fails that tool before spawn. Declared tool processes receive a 192 MiB V8 heap limit and a 1 MiB output limit by default. On systemd-based Linux hosts, Arisa also places each process tree in a 384 MiB memory-limited cgroup with swap disabled and raises that tree's OOM-kill priority, so browser descendants fail with the tool instead of exhausting the worker. Exceeding a limit terminates only that process tree and produces an uncertain tool result. Manifests may set `maxHeapMb`, `maxMemoryMb`, and `maxOutputBytes`. An opt-in tool may set `deduplicateConcurrent: true` to join only exact concurrent duplicates from the same chat. Override admission with `toolExecution.minAvailableMemoryMb`, `toolExecution.maxWorkerRssMb`, and `toolExecution.maxSwapUsedPercent`, or capacities with `toolExecution.defaultCapacity`, `toolExecution.capacities`, and `toolExecution.maxQueuedPerClass`. Arisa logs pressure rejections, queue waits, joined duplicates, and worker RSS peaks.
