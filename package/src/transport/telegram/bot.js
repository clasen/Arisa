...
import { buildReactionPrompt, buildStartupMessage } from "./prompt-builders.js";
import { createChatStateStore, resolveTelegramBusyMessageMode } from "./chat-queue.js";
import { createTelegramPromptController } from "./telegram-prompt-controller.js";
export {
...
export async function createTelegramBot({ config, artifactStore, toolRegistry, taskStore, agentManager, saveConfig, updateConfig, doctor, checkUpdates, updateCore, updateTools, requestRestart, logger }) {
  ...
  const {
    ...
    enqueuePrompt,
    handleNewCommand
  } = createTelegramPromptController({
    config,
    api: bot.api,
    artifactStore,
    toolRegistry,
    ...
    ensureQueuedTyping: ensureQueuedTelegramTyping,
    withTyping,
    resolveBusyMessageMode: resolveIncomingBusyMessageMode
  });
